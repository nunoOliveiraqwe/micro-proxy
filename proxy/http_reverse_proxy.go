...
func buildNetListeners(ipv4BindIf, ipv6BindIf string, port int) []net.Listener {
	...
	if ipv4BindIf == "" && ipv6BindIf == "" {
		zap.S().Warnf("No bind interfaces specified, listening on all interfaces on port :0")
		addr := fmt.Sprintf(":%d", port)
		listen, err := net.Listen("tcp", addr)
		if err != nil {
			zap.S().Errorf("Failed to listen on default interface: %s", err)
		} else {
			lns = append(lns, listen)
		}
	}
